...
declare global {
  interface Window {
    electronAPI: {
      ...
      vpnInstall: () => Promise<{ success: boolean; error?: string }>
      ...
      vpnConnect: (config: string) => Promise<{ success: boolean; tunnelName?: string; configPath?: string; needsInstall?: boolean; error?: string }>
      vpnDisconnect: () => Promise<{ success: boolean; error?: string }>
